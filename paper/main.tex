...
the accumulated shear, and the modal energy $S=P^2+Q^2$ is
...
component realizes the horizontal generator of $H_1(\T^2;\ZZmod)$, has
...
compensated-gap shear when horizontal winding is allowed. A cylinder analogue holds for time-independent mean
...
shown to exist together with its value). The flux is the pairing of the
undetermined mean shear with the modal energy weight of the wave; its
status as an observable, the normalization of the velocity field, and its
behavior under Galilean boosts are discussed in
...
\cite{KNSS2009} for the Navier--Stokes equations and of the axisymmetric
...
\begin{hypothesis}[No horizontal winding]\label{hyp:conf}
...
component of the level set $\{W(\cdot,\cdot,\tau)=c\}$ realizes the
horizontal generator of $H_1(\T^2;\ZZmod)$.
\end{hypothesis}

Hypothesis~\ref{hyp:conf} forbids exactly one topological event: a
regular level component winding around the torus in the $\xi$-direction.
Vertical winding, and arbitrary behavior at critical values, are allowed;
the conventions are discussed in Remark~\ref{rem:conventions}.
...
particular the \emph{modal energy} $S:=P^2+Q^2=P_0^2+Q_0^2$ is
...
satisfying Hypotheses~\ref{hyp:v} and~\ref{hyp:conf}. Then the ancient
mean flux exists and vanishes: $\Fbar=0$.
\end{theorem}
...
Indeed, a contractible component is null-homotopic, hence trivial in
$H_1(\T^2;\ZZmod)$, so confinement implies Hypothesis~\ref{hyp:conf}; and
...
spectral gap realizes the horizontal generator), and a
...
and a finite union of components none of which carries the core class
cannot separate --- so some component realizes the horizontal generator,
violating Hypothesis~\ref{hyp:conf}. This pins the mean profile,
...
Proposition~\ref{prop:counterexample}, and allowing horizontal winding
...
Steady cells ($b_Z\equiv0$) and traveling waves are the classical
zero-flux members of the ancient class. For \emph{steady} hydrostatic
...
stagnation are shear flows (see also the references there). The present
result concerns the genuinely time-dependent ancient regime, where the
freedom of the undetermined mean sector is the central difficulty; with
the classification stated in Fourier form, the theorem covers each
single-mode stratum in full. The multi-mode problem remains open --- to
the best of the author's knowledge, no Liouville-type result is
available for the time-dependent ancient regime beyond the single-mode
strata --- and appears to be of a different nature; we state it
precisely in Section~\ref{sec:open}. We do not claim any consequence at
the level of the primitive (axisymmetric Navier--Stokes) equations.
...
reading --- forbidding horizontal winding for every component on which
$\nabla W\neq0$, regardless of the value --- the theorems also hold,
with slightly lower regularity requirements
(Remark~\ref{rem:regularity}). (iii) Only the $\ZZmod$-class of the
horizontal generator is used, which is why the hypothesis is phrased in
$H_1(\T^2;\ZZmod)$.
\end{remark}
...
$k$. In particular the \emph{modal energy}
\[
...
$c^*=b(Z^*)$, $Z^*\in J$, the level component through $(\cdot,Z^*)$ is
exactly the horizontal circle $\{Z=Z^*\}$: regular (since
$|\nabla W|=|b'(Z^*)|\neq0$ there) and realizing the horizontal
generator of $H_1(\T^2;\ZZmod)$. The gap region is invariant under the
...
zero circle, that circle is itself a regular level component realizing
the horizontal generator and the hypothesis already fails (proof of
Lemma~\ref{lem:annular}). The theorem excludes this candidate not at
...
component of $L$ represents the core class
$[\T_\xi\times\{\mathrm{pt}\}]\in H_1(\mathcal A;\ZZmod)$; under the
inclusion $\mathcal A\hookrightarrow\T^2$ this class maps to the
horizontal generator of $H_1(\T^2;\ZZmod)$.
\end{lemma}
...
$H_1(\mathcal A;\ZZmod)$. The inclusion
$\mathcal A\hookrightarrow\T^2$ sends the core circle to a horizontal
circle, whose class is the horizontal generator of $H_1(\T^2;\ZZmod)$.
\end{proof}
...
theorem. By Lemma~\ref{lem:annulus-top}, some component of $L$ realizes
the horizontal generator of $H_1(\T^2;\ZZmod)$ --- and $\lambda$ is a
...
$\xi$; choosing such a $\lambda$ that is moreover a Sard-regular value
of $W$ gives a regular level component realizing the horizontal
generator, and Hypothesis~\ref{hyp:conf} fails under either reading of
...
Sard selection, and it is exactly these that wind.
\end{proof}
...
\end{scope}
\end{tikzpicture}
\caption{Level-set portraits. (a)~Regular levels of the counterexample
...
Sard-regular intermediate level separates the ends and some component
winds horizontally
...
the gap realize the horizontal generator.}
\label{fig:portraits}
\end{figure}
...
\label{rem:unified}
The horizontal-generator separation argument appears three times in
...
On the class \eqref{eq:full-class}, $\mean{u^3}\equiv0$ and the modal
energy $S$ is pointwise conserved (Lemma~\ref{lem:class-cartesian}).
...
component of $W(\cdot,\cdot,\tau)$ realizes the generator of
$H_1(\T_\xi\times\R;\ZZmod)$, then $b'\equiv0$: the mean profile is
...
since $|\nabla W|=|b'(Z^*)|\neq0$; a \emph{complete} component since
$Z^*$ is an isolated root of $b(Z)=c^*$ by $b'(Z^*)\neq0$, so the level
contains no other nearby points) which is a circle in
$\T_\xi\times\R$ generating $H_1(\T_\xi\times\R;\ZZmod)$.
...
produces a component realizing the generator of
$H_1(\T_\xi\times\R;\ZZmod)$, violating the hypothesis. Hence $c_0=0$, i.e.\ $b'\equiv0$; the flux statement follows from
Lemma~\ref{lem:flux}.
\end{proof}
...
\begin{proposition}[Allowing horizontal winding: the compensated-gap
...
Hypothesis~\ref{hyp:conf}: horizontal regular levels inside the gap
realize the horizontal generator (Figure~\ref{fig:portraits}(c)).
\end{proposition}
...
$\{Z=Z^*\}$, realizing the horizontal generator of $H_1(\T^2;\ZZmod)$.
\end{proof}
...
(this is a statement about these witnesses, not about general members:
regular components of $\cos(\xi-Z)$ have class $(1,1)$, allowed by
Hypothesis~\ref{hyp:conf}, while for its $k=2$ cover $\cos(2\xi-Z)$
they have class $(1,2)\equiv(1,0)$ mod~$2$, the forbidden horizontal
generator). First,
...
scripts are citable as ancillary files of the arXiv submission; a DOI
deposit of the bundle is planned at publication. None is load-bearing
...
asserts its checks and exits with nonzero status on any failure.
...
battery: six single-site mathematical corruptions of the checkers ---
including a drift of the parameter $\varepsilon$ itself, caught by the
gates anchored to literal reference constants --- are applied to
temporary copies, and each corrupted checker is asserted to exit
nonzero.
\end{itemize}
...
ESAIM Math.\ Model.\ Numer.\ Anal.\ \textbf{33} (1999), no.~5,
965--970.
