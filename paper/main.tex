...
undetermined mean shear with the squared modal amplitude of the wave; it
is the vertical transport $-\mean{u^2v}$ of the horizontal kinetic energy
$\tfrac12u^2$ by the transverse velocity $v$ --- a Reynolds-type second
moment, not the flux of a conserved density (Remark~\ref{rem:observable}),
and the natural scalar diagnostic of the mean sector against the wave. Its
...
$\mean{u^2}_\xi=\tfrac{k^2}2S$ is the horizontal kinetic energy of the
mode) is time-independent, and \eqref{eq:full-class} is \emph{exactly} the
...
$W=b(Z)$, so the level set there is
$\T_\xi\times\{Z\in I:b(Z)=c^*\}$; since $b'\neq0$ on $J$ the value $c^*$
is attained at the single point $Z^*\in J$, and the compact circle
$\{Z=Z^*\}$ --- regular, since $|\nabla W|=|b'(Z^*)|\neq0$ --- is thus a
full connected component of the level, freely homotopic to a horizontal
circle. The gap region is invariant under the
...
theorem. By Lemma~\ref{lem:annulus-top}, some component of $L$ is freely
homotopic to a horizontal circle --- and $\lambda$ is a
